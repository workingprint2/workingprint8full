<?php
/**
 * Bean Conditions Class for Widget Areas
 *
 *
* @package WordPress
...
class Bean_Conditions {
	...
	function __construct () 
	{
		...
		add_action( 'wp_ajax_beansidebars-toggle-advanced-items', array( &$this, 'ajax_toggle_advanced_items' ) );
	...
	function get_conditions () 
	{
		...
		$this->conditions = apply_filters( 'bean_conditions', $this->conditions );
	...
	} //END function determine_conditions () 
	...
	function setup_default_conditions_reference () 
	{
		...
		foreach ( $post_types as $k => $v ) {
			if ( ! post_type_supports( $k, 'beansidebars' ) ) { continue; }
			...
			foreach ( $post_types as $k => $v ) {
				...
				if ( $v->has_archive ) {
					$conditions['post_types'][$token] = array(
										'label' => sprintf( __( '%s Posts Archive', 'bean' ), $v->labels->name ), 
	...
	} //END function setup_default_conditions_reference ()
	...
	} //END function is_post_type_archive ()
	...
	function meta_box_setup () {		
		add_meta_box( 'bean-conditions', $this->meta_box_settings['title'], array( &$this, 'meta_box_content' ), $this->token, 'normal', 'low' );
	} // End meta_box_setup()
	...
	function meta_box_content () 
	{
		...
		$html .= '<input type="hidden" name="bean_' . $this->token . '_conditions_noonce" id="bean_' . $this->token . '_noonce" value="' . wp_create_nonce( plugin_basename(__FILE__) ) . '" />';
		...
		if ( count( $this->conditions_reference ) > 0 ) {
			...
			$html .= '<div id="taxonomy-category" class="categorydiv tabs bean-conditions">' . "\n";
				...
				foreach ( $this->conditions_reference as $k => $v ) {
					...
					if ( isset( $this->conditions_headings[$k] ) ) {
						$html .= '<li class="' . esc_attr( $class ) . '"><a class="button-secondary" href="#tab-' . $k . '">' . $this->conditions_headings[$k] . '</a></li>' . "\n";
					}
				}
			...
			foreach ( $this->conditions_reference as $k => $v ) 
			{
				...
				$tab = apply_filters( 'bean_conditions_tab_' . esc_attr( $k ), $tab );
				$html .= $tab;
		...
		do_action( 'bean_conditions_meta_box', $k, $v );
	...
	function meta_box_save ( $post_id ) 
	{
		...
		// Verify
		if ( ( get_post_type() != $this->token ) || ! wp_verify_nonce( $_POST['bean_' . $this->token . '_conditions_noonce'], plugin_basename(__FILE__) ) ) {  
			return $post_id;  
		}
	...
	} //END function meta_box_save ( $post_id ) 
	...
	private function show_advanced_items () 
	{
		...
		return $response;
	} //END private function show_advanced_items ()
	...
	} //END public function ajax_toggle_advanced_items ()
	...
	public function enqueue_scripts () 
	{
		...
		if ( in_array( $pagenow, array( 'post.php', 'post-new.php' ) ) ) {
			wp_register_script( $this->token . '-admin', BEAN_FRAMEWORK_URL . '/widget-areas/bean-admin-widget-areas.js', array( 'jquery', 'jquery-ui-tabs' ), '1.0.0', true );
			...
			$ajax_vars = array( 'beansidebars_advanced_noonce' => wp_create_nonce( 'beansidebars_advanced_noonce' ) );

			$data = array_merge( $translation_strings, $ajax_vars );

			wp_localize_script( $this->token . '-admin', 'beansidebars_localized_data', $data );
		}
